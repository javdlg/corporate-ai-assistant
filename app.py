...
from src.graph.workflow import app as workflow_app  # noqa: E402
...
with st.sidebar:
    ...
    if os.path.exists(RAW_DIR):
        files = [
            ...
            if os.path.isfile(os.path.join(RAW_DIR, f)) and f != ".gitkeep"
        ]
        if files:
            for file in files:
                st.markdown(
                    f'<div class="doc-item">📄 {file}</div>', unsafe_allow_html=True
                )
    ...
    if st.button(
        ...
        with st.spinner(
            "Ejecutando pipeline de ingesta, fragmentación e indexación en ChromaDB..."
        ):
            try:
...
# ==========================================
st.markdown(
    '<div class="main-title">🏢 Asistente AI de OmniCorp</div>', unsafe_allow_html=True
)
st.markdown(
    '<div class="subtitle">Interactúa con la base de conocimiento oficial de la empresa para resolver dudas operativas, políticas o financieras.</div>',
    unsafe_allow_html=True,
)
